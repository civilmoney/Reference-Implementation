...
namespace CM.JS {
    ...
    public class FindAccountRequest {
        ...
        internal Controls.ServerProgressIndicator UI;
    ...
    internal class Client {
        ...
        private void FindResponsiblePeersForAccount(List<Peer> servers, string id, IAsyncRequest req, int numberOfPlacesToLook, Action<List<Peer>> onResult, Controls.ServerProgressIndicator ui) {
            CachedResponsiblePeers cached;
            if (_CachedResponsiblePeers.TryGetValue(id, out cached)
                && cached.Age.TotalSeconds < 120) {
                if (ui != null)
                    ui.AppendPeers(cached.Peers.ToArray());
        ...
        private void QueryAuthoritiveServerImpl(string server, List<string> tried, AsyncRequest<HttpRequest> onResult) {
            ...
            r.onreadystatechange = (ev) => {
                if (r.readyState == dom.XMLHttpRequest.DONE_Static) {
                ...
                } else if (r.readyState == dom.XMLHttpRequest.HEADERS_RECEIVED_Static) {
                    onResult.Item.Status = r.statusText;
                }
            };
            try {
        ...
        private void QueryAuthoritiveServerTryNextOrGiveUp(List<string> tried, dom.XMLHttpRequest r, AsyncRequest<HttpRequest> onResult) {
            string next = null;
        ...
        private void TryFindOnNetwork<T>(List<Peer> servers, string path, IAsyncRequest req, Action<T> onResult, Controls.ServerProgressIndicator ui = null)
            ...
            FindResponsiblePeersForPath(servers, path, req, (peers) => {
                ...
                double total = peers.Count * 2;
                var step = 0;
                for (int i = 0; i < peers.Count; i++) {
                    if (ui != null)
                        ui.Update(peers[i], ServerProgressIndicatorStatus.Waiting, SR.LABEL_STATUS_CONNECTING);
                    req.UpdateProgress((int)((++step / total)*100));
                    peers[i].SendAndReceive("GET", null, new string[] { path }, (p, res) => {
                        ...
                        if (ui != null) {
                            ui.Update(p,
                                res.Response.IsSuccessful ? ServerProgressIndicatorStatus.Success : ServerProgressIndicatorStatus.Unknown,
    ...
    internal class PutRequest {
        ...
        internal ServerProgressIndicator UI;
        ...
        public void UpdateUIProgress() {
            ...
            for (int i = 0; i < Statuses.Count; i++) {
                var st = Statuses[i];
                var glyph = st.IsCommitted ?
                    ServerProgressIndicatorStatus.Success
                    : st.IsInError ? ServerProgressIndicatorStatus.Error
                    : ServerProgressIndicatorStatus.Waiting;
