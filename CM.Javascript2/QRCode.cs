...
//
// QRCode is written by https://github.com/codebude/QRCoder - MIT License.
// Tweaked for performance and Bridget.NET/javascript portability.
//

#endregion Other Credits
...
namespace CM.JS {
    ...
    internal static class QRCode {
        ...
        public static string GenerateQRCode(string text, int width, int height, string domWidth) {
            var gen = new QRCodeGenerator();
            var qrCodeData = gen.CreateQrCode(text, QRCodeGenerator.ECCLevel.L, false);
            int unitsPerModule = (int)Math.Floor(Convert.ToDouble(Math.Min(width, height)) / qrCodeData.ModuleMatrix.Count);
            var size = qrCodeData.ModuleMatrix.Count * unitsPerModule;
            var s = new StringBuilder(
                $"<svg width=\"{domWidth}\" viewbox=\"0 0 {width} {height}\" xmlns=\"http://www.w3.org/2000/svg\"><style>rect {{ stroke-width:0; stroke: none; fill: #000000; }}</style>");
            var offset = (int)Math.Floor(width / 2.0 - size / 2.0);
            for (int x = 0; x < size; x = x + unitsPerModule) {
                for (int y = 0; y < size; y = y + unitsPerModule) {
                    var module = qrCodeData.ModuleMatrix[(y + unitsPerModule) / unitsPerModule - 1][(x + unitsPerModule) / unitsPerModule - 1];
                    if (!module)
                        continue;
                    s.AppendLine($"<rect x=\"{ x + offset}\" y=\"{ y + offset }\" width=\"{unitsPerModule}\" height=\"{unitsPerModule}\" />");//fill=""" + (module ? darkColorHex : lightColorHex) + @"""
                }
            }
            s.Append("</svg>");
            return s.ToString();
        ...
        private class QRCodeGenerator {
            ...
            public QRCodeData CreateQrCode(string plainText, ECCLevel eccLevel, bool utf8BOM = false) {
                ...
                var encoding = EncodingMode.Byte;
                ...
                var codedText = PlainTextToBinary(plainText, encoding, utf8BOM);
                var dataInputLength = codedText.Length / 8;
                ...
                ECCInfo eccInfo = new ECCInfo(); // = capacityECCTable.Single(x => x.Version == version && x.ErrorCorrectionLevel.Equals(eccLevel));
                for (int i = 0; i < capacityECCTable.Count; i++)
                    if (capacityECCTable[i].Version == version && capacityECCTable[i].ErrorCorrectionLevel.Equals(eccLevel)) {
                        eccInfo = capacityECCTable[i];
                        break;
                    }
                ...
                List<Point> alignmentPatternLocations = null;
                // alignmentPatternTable.Where(x => x.Version == version).Select(x => x.PatternPositions).First()
                for (int i = 0; i < alignmentPatternTable.Count; i++)
                    if (alignmentPatternTable[i].Version == version) {
                        alignmentPatternLocations = alignmentPatternTable[i].PatternPositions;
                        break;
                    }
                ModulePlacer.PlaceAlignmentPatterns(ref qr, alignmentPatternLocations, ref blockedModules);
            ...
            private string[] BinaryStringToBitBlockList(string bitString) {
                // return bitString.ToList().Select((x, i) => new { Index = i, Value = x })
                //     .GroupBy(x => x.Index / 8)
                //     .Select(x => String.Join("", x.Select(v => v.Value.ToString()).ToArray()))
                //     .ToList();
                int len = bitString.Length;
                string[] tmp = new string[(int)Math.Ceiling(len / 8.0)];
                int idx = 0;
                for (int i = 0; i < tmp.Length && idx < len; i++) {
                    var s = "";
                    for (int x = 0; x < 8 && idx < len; x++) {
                        s += bitString[idx++];
                    }
                    tmp[i] = s;
                }
            ...
            private string[] CalculateECCWords(string bitString, ECCInfo eccInfo) {
                ...
                for (int i = 0; i < count; i++) {
                    if (leadTermSource.PolyItems[0].Coefficient == 0) {   // First coefficient is already 0, simply remove it and continue
                ...
                var ar = new string[leadTermSource.PolyItems.Count];
                for (int i = 0; i < leadTermSource.PolyItems.Count; i++)
                    ar[i] = DecToBin(leadTermSource.PolyItems[i].Coefficient, 8);
                return ar;
            ...
            private Polynom ConvertToAlphaNotation(Polynom poly) {
                ...
                int count = src.Count;
                var ar = new PolynomItem[count];
                for (int i = 0; i < count; i++) {
                    //newPoly.PolyItems.Add(new PolynomItem() {
                    var item = src[i];
                    ar[i].Coefficient = (item.Coefficient != 0 ? GetAlphaExpFromIntVal(item.Coefficient) : 0);
                    ar[i].Exponent = item.Exponent;
                    //});
                }
                Polynom newPoly = new Polynom();
                newPoly.PolyItems = new List<PolynomItem>(ar);
                return newPoly;
            }

            private Polynom ConvertToDecNotation(Polynom poly) {
                var src = poly.PolyItems;
                int count = poly.PolyItems.Count;
                var ar = new PolynomItem[count];
                for (int i = 0; i < poly.PolyItems.Count; i++) {
                    //newPoly.PolyItems.Add(new PolynomItem() {
                    var item = src[i];
                    ar[i].Coefficient = GetIntValFromAlphaExp(item.Coefficient);
                    ar[i].Exponent = item.Exponent;
                    // });
                }
                Polynom newPoly = new Polynom();
            ...
            private void CreateAlphanumEncDict() {
                ...
                for (int i = 0; i < alphanumEncTable.Length; i++)
                    alphanumEncDict[alphanumEncTable[i]] = i;
            }
            ...
            private int GetAlphaExpFromIntVal(int intVal) {
                ...
                for (int i = 0; i < galoisField.Count; i++) {
                    if (galoisField[i].IntegerValue == intVal)
                        return galoisField[i].ExponentAlpha;
                }
                throw new Exception();
            }
            ...
            private EncodingMode GetEncodingFromPlaintext(string plainText) {
                var enc = EncodingMode.Numeric;
                for (int i = 0; i < plainText.Length; i++) {
                    var c = plainText[i];
                    if (!Char.IsDigit(c))
                        enc = EncodingMode.Alphanumeric;
                    if (!Char.IsLetterOrDigit(c))
                        enc = EncodingMode.Byte;
                }
                return enc;
            ...
            private int GetIntValFromAlphaExp(int exp) {
                ...
                for (int i = 0; i < galoisField.Count; i++) {
                    if (galoisField[i].ExponentAlpha == exp)
                        return galoisField[i].IntegerValue;
                }
                throw new Exception();
            }
            ...
            private int GetVersion(int length, EncodingMode encMode, ECCLevel eccLevel) {
                ...
                for (int i = 0; i < capacityTable.Count; i++) {
                    var details = capacityTable[i].Details;
                    if (version > capacityTable[i].Version)
                        for (int x = 0; x < details.Count; x++) {
                            if (details[x].ErrorCorrectionLevel == eccLevel
                                && details[x].CapacityDict[encMode] >= length) {
                                version = capacityTable[i].Version;
                            }
                        }
                }
                return version;
            ...
            private Polynom MultiplyAlphaPolynoms(Polynom polynomBase, Polynom polynomMultiplier) {
                ...
                var exponentsToGlue = new List<int>();
                var dic = new Dictionary<int, List<PolynomItem>>();
                for (int i = 0; i < resultPolynom.PolyItems.Count; i++) {
                    var exp = resultPolynom.PolyItems[i].Exponent;
                    List<PolynomItem> ar;
                    if (!dic.TryGetValue(exp, out ar)) {
                        ar = new List<PolynomItem>();
                        dic[exp] = ar;
                    }
                    ar.Add(resultPolynom.PolyItems[i]);
                }
                foreach (var dics in dic.Values) {
                    if (dics.Count > 1) {
                        exponentsToGlue.Add(dics[0].Exponent);
                    }
                }
                ...
                for (int i = 0; i < resultPolynom.PolyItems.Count; i++) {
                    if (exponentsToGlue.Contains(resultPolynom.PolyItems[i].Exponent)) {
                ...
                resultPolynom.PolyItems.Sort((a, b) => { return a.Exponent.CompareTo(b.Exponent) * -1; });
            ...
            private Polynom XORPolynoms(Polynom messagePolynom, Polynom resPolynom) {
                ...
                int longCount = longPoly.Count;
                int shortCount = shortPoly.Count;
                int firstExponent = messagePolynom.PolyItems[0].Exponent;
                PolynomItem polItemRes = new PolynomItem(); // bridge optimisation for slow struct setup
                for (int i = 0; i < longCount; i++) {
                    polItemRes.Coefficient = longPoly[i].Coefficient ^ (shortCount > i ? shortPoly[i].Coefficient : 0);
                    polItemRes.Exponent = firstExponent - i;
                    resultPolynom.PolyItems.Add(polItemRes);
                }
            ...
            private static class ModulePlacer {
                ...
                public static int MaskCode(ref QRCodeData qrCode, int version, ref List<Rectangle> blockedModules) {
                    ...
                    MaskPattern.PatternDelegate curPattern = null;
                    int patternIndex = -1;
                    ...
                    for (int i = 0; i < MaskPattern.Patterns.Length; i++) {
                        var pattern = MaskPattern.Patterns[i];
                        var qrTemp = new QRCodeData(version);
                        for (int y = 0; y < size; y++) {
                            var dst = qrTemp.ModuleMatrix[y];
                            var src = qrCode.ModuleMatrix[y];
                        ...
                        for (int x = 0; x < size; x++) {
                            for (int y = 0; y < size; y++) {
                                rect.X = x;
                                rect.Y = y;
                                if (!IsBlocked(rect, blockedModules)) {
                                    qrTemp.ModuleMatrix[y][x] ^= (bool)pattern(x, y);
                                }
                            }
                        }
                    ...
                    for (int x = 0; x < size; x++) {
                        for (int y = 0; y < size; y++) {
                            ...
                            rect.Y = y;
                            if (!IsBlocked(rect, blockedModules)) {
                                qrCode.ModuleMatrix[y][x] ^= curPattern(x, y);// (bool)patterMethod.Invoke(null, new object[] { x, y });
                            }
                        }
                    }
                ...
                public static void PlaceDataWords(ref QRCodeData qrCode, string data, ref List<Rectangle> blockedModules) {
                    ...
                    for (int x = size - 1; x >= 0; x = x - 2) {
                        ...
                        for (int yMod = 1; yMod <= size; yMod++) {
                            ...
                            if (up) {
                                y = size - yMod;
                                if (counter < max && !IsBlocked(new Rectangle(x, y, 1, 1), blockedModules))
                                    qrCode.ModuleMatrix[y][x] = data[counter++] == '1';//datawords.Dequeue();
                                if (counter < max && x > 0 && !IsBlocked(new Rectangle(x - 1, y, 1, 1), blockedModules))
                                    qrCode.ModuleMatrix[y][x - 1] = data[counter++] == '1';//datawords.Dequeue();
                            } else {
                                y = yMod - 1;
                                if (counter < max && !IsBlocked(new Rectangle(x, y, 1, 1), blockedModules))
                                    qrCode.ModuleMatrix[y][x] = data[counter++] == '1';//datawords.Dequeue();
                ...
                private static class MaskPattern {
                    ...
                    public static readonly PatternDelegate[] Patterns = new PatternDelegate[] {
                        Pattern1,
                        Pattern2,
                        Pattern3,
                        Pattern4,
                    ...
                    public delegate bool PatternDelegate(int x, int y);
                    ...
                    public static int Score(ref QRCodeData qrCode) {
                        ...
                        for (int i = 0; i < qrCode.ModuleMatrix.Count; i++) {
                            var ar = qrCode.ModuleMatrix[i];
                            for (int x = 0; x < ar.Length; x++) {
                                if (ar[x])
                                    blackModules++;
            ...
            private class Polynom {
                ...
                public override string ToString() {
                    ...
                    for (int i = 0; i < PolyItems.Count; i++)
                        sb.Append("a^" + PolyItems[i].Coefficient + "*x^" + PolyItems[i].Exponent + " + ");
