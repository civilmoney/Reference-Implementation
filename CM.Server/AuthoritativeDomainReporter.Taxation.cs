...
namespace CM.Server {
    internal partial class AuthoritativeDomainReporter {
        ...
        [Flags]
        private enum TaxRevenueFlags : int {
            None = 0,
        ...
        /// </summary>
        private enum TransactionStatus : int {
            Unknown = 0,
            Settled,
            Cancelled,
        }
        ...
        /// </summary>
        private SummaryItemCache GetTaxSummaryItems(string region, DateTime from, DateTime to) {
        ...
        public async Task HandleApiGetTaxData(SslWebContext context, string region) {
            // input validation
            if (ISO31662.GetName(region) == null) {
                await context.ReplyAsync(HttpStatusCode.NotFound, null, "Region not found");
                return;
            }

            DateTime from, to;

            if (!Helpers.DateFromISO8601(context.QueryString["from"] ?? String.Empty, out from)) {
                await context.ReplyAsync(HttpStatusCode.BadRequest, null, "Invalid 'from' date string.");
                return;
            }
            if (!Helpers.DateFromISO8601(context.QueryString["to"] ?? String.Empty, out to)) {
                await context.ReplyAsync(HttpStatusCode.BadRequest, null, "Invalid 'to' date string.");
                return;
            }
            if (to <= from) {
                await context.ReplyAsync(HttpStatusCode.BadRequest, null, "Invalid 'to' date value. Must be greater than from.");
                return;
            }

            var ar = GetTaxSummaryItems(region, from, to).Items;

            int startAt;
            int.TryParse(context.QueryString["startat"], out startAt);
            int max;
            int.TryParse(context.QueryString["max"], out max);
            if (max == 0)
                max = 1000;
            var s = new StringBuilder();
            for (int i = startAt; i < Math.Min(ar.Count, startAt + max); i++) {
                var t = ar[i];
                s.Append(t.Date.ToString("s") + " " + t.ID + " " + t.Revenue + " " + ((t.Flags & TaxRevenueFlags.Ineligible) == TaxRevenueFlags.Ineligible ? "NT" : "OK") + "\r\n");
            }
            await context.ReplyAsync(HttpStatusCode.OK, null, s.ToString());
        }

        public async Task HandleApiGetTaxRevenue(SslWebContext context, string region) {
            // input validation
            if (ISO31662.GetName(region) == null) {
                await context.ReplyAsync(HttpStatusCode.NotFound, null, "Region not found");
                return;
            ...
            var ar = GetTaxSummaryItems(region, DateTime.UtcNow.Date.AddYears(-2), DateTime.UtcNow.Date.AddDays(1));
            decimal total = 0;
            int count = 0;
            for (int i = 0; i < ar.Items.Count; i++) {
                if ((ar.Items[i].Flags & TaxRevenueFlags.Ineligible) == TaxRevenueFlags.Ineligible)
                    continue;
                total += ar.Items[i].Revenue;
                count++;
            }

            var json = "{ \"count\":" + count + ", \"revenue\":" + total + ", \"lastUpdatedUtc\":\"" + Helpers.DateToISO8601(ar.GeneratedUtc) + "\" }";
        ...
        /// </summary>
        private struct SummaryItem : IComparable<SummaryItem> {
        ...
        /// </summary>
        private class RevenueItem : IComparable<RevenueItem> {
        ...
        /// </summary>
        private class RevenueItemCache {
        ...
        /// </summary>
        private class SummaryItemCache {
