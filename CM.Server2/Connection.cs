...
namespace CM.Server {
    ...
    internal delegate Task ProcessRequestDelegate(Connection conn, Message m);
    ...
    public class Connection : IDisposable {
        ...
        /// </summary>
        internal Connection(Server sender, WebSocket socket,
            ...
            RemoteEndpoint = new IPEndPoint(context.GetIPAddress(sender.Configuration), _Context.Connection.RemotePort);
        ...
        /// </summary>
        public Connection(IPEndPoint remoteEndpoint, WebSocket socket) : this() {
        ...
        public bool IsConnected {
            get {
                var ctx = _Outbound ?? _Inbound;
                return (ctx != null
                    && (ctx.State == WebSocketState.Open
                    || ctx.State == WebSocketState.Connecting));
            }
        }
        ...
        public async Task<bool> Reply(Message original, CMResult status, Message playload = null, params string[] args) {
            ...
            try {
                ...
                hasLock = true;
                await _Inbound.SendAsync(new ArraySegment<byte>(b), WebSocketMessageType.Text, true, CancellationToken.None)
