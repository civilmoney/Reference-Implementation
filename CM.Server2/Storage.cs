...
namespace CM.Server {
    ...
    internal partial class Storage : IDisposable {
        ...
        private System.Collections.Concurrent.ConcurrentDictionary<string, ConcurrentDictionary<Connection, int>> _Subscriptions;
        ...
        public Storage(string folder, DistributedHashTable dht, Log log) {
            ...
            _Subscriptions = new System.Collections.Concurrent.ConcurrentDictionary<string, ConcurrentDictionary<Connection, int>>();
        }
        ...
        public void AddSubscription(Connection conn, string id) {
            ...
            if (!Monitor.TryEnter(conn.SubscriptionSync, 1000)) {
            ...
            try {
                ...
                Monitor.Exit(conn.SubscriptionSync);
            }

            if (!_Subscriptions.TryGetValue(id, out var connections)) {
                connections = new ConcurrentDictionary<Connection, int>();
        ...
        public async Task<CMResult> Commit(string token,
            ...
            if (!IsOperatingInTestMode && !skipNetworkCorroboration) {
                var peers = new System.Collections.Concurrent.BlockingCollection<string>();
        ...
        void CollectSubscribers(List<Connection> ar, string id) {
            if (_Subscriptions.TryGetValue(id.ToLower(), out var connections)
                    && connections != null) {
                var toRem = new List<Connection>();
                foreach (var kp in connections) {
                    if (!kp.Key.IsConnected) {
                        toRem.Add(kp.Key);
                    } else {
                        ar.Add(kp.Key);
                    }
        ...
        async void SendSubscribeNotifications(IStorable item) {
            ...
            if (item is Account) {
                ...
                CollectSubscribers(ar, a.ID.ToLower());
            } else if (item is Transaction) {
                var t = item as Transaction;
                CollectSubscribers(ar, t.PayeeID.ToLower());
                CollectSubscribers(ar, t.PayerID.ToLower());
            }
        ...
        public async Task<PutResult> Put(IStorable item) {
            ...
            if (item is Account) {
                ...
                req.Item.New = t;
                if (!IsOperatingInTestMode)
                    req.Item.Old = await TryFindOnNetwork<Transaction>(t.Path).ConfigureAwait(false);
                else
                ...
                req.Item.New = v;
                if (!IsOperatingInTestMode)
                    req.Item.Old = await TryFindOnNetwork<Vote>(v.Path).ConfigureAwait(false);
                else
        ...
        public async Task<T> TryFindOnNetwork<T>(string path)
            ...
            await FindResponsiblePeersForPath(peers, path).ConfigureAwait(false);

            var copies = new System.Collections.Concurrent.BlockingCollection<T>();

            await peers.ForEachAsync(3,
                async (ep) => {
                    using (var conn = await _DHT.Connect(ep).ConfigureAwait(false)) {
                        if (conn.IsValid) {
                            return await conn.Connection.SendAndReceive("GET", null, path).ConfigureAwait(false);
                        } else {
                            return null;
                        }
                    }
            ...
            T account;
            Helpers.CheckConsensus(copies.ToList(), out account);
            return account;
        }

        internal async Task FindResponsiblePeersForAccount(System.Collections.Concurrent.BlockingCollection<string> peers, string id) {
        ...
        private async Task FindResponsiblePeersForPath(System.Collections.Concurrent.BlockingCollection<string> peers, string path) {
        ...
        private async Task Validate(AsyncRequest<ValidateRequest<Transaction>> e, List<string> notifications) {
            ...
            if (ver.Result != CMResult.S_OK) {
                _Log.DebugDump("Invalid payer signature", t.ToContentString());
            ...
            if (t.PayeeSignature != null) {
                ...
                if (ver.Result != CMResult.S_OK) {
                    _Log.DebugDump("Invalid payee signature", t.ToContentString());
        ...
        private async Task Validate(AsyncRequest<ValidateRequest<Account>> e, List<string> notifications) {
            ...
            if (a.AccountSignature == null) {
                ...
                _Log.DebugDump("Missing account signature", a.ToContentString());
                return;
            }
            ...
            if (verify.Result != CMResult.S_OK) {
                e.Completed(CMResult.E_Account_Signature_Error);
                _Log.DebugDump("Invalid account signature", a.ToContentString());
                return;
            }
        ...
        private async Task Validate(AsyncRequest<ValidateRequest<Vote>> e, List<string> notifications) {
            ...
            if (verify.Result != CMResult.S_OK) {
                ...
                _Log.DebugDump("Invalid vote signature", v.ToContentString() + "\r\n========\r\n" + a.ToContentString());
                return;
            }
