...
test("javascript AES (1)", function () {
    var src = System.Text.Encoding.UTF8.GetBytes("hello");
    var key = System.Convert.fromBase64String("bRz+T0fMZxbW3uXr7iU3rglWCHP5CXeN268kllaD7IQ=");
    var iv = System.Convert.fromBase64String("eQ1rQdFUYJ54w3FU3Btvqw==");

    //var key_schedule = System.Array.init(60, 0, System.Byte);
    //var keySize = (key.length * 8) | 0;
    //CM.Cryptography.AES.aes_key_setup(key, key_schedule, keySize);
    //equal(System.Convert.toBase64String(key_schedule), "", "ks");

    var enc = CM.Cryptography.AES.Encrypt(src, key,iv);
...
*/
test("Bridge date bugs", function () {
    var d = System.DateTime.getUtcNow();
    var s = CM.Helpers.DateToISO8601(d);
    var d2 = {};
    CM.Helpers.DateFromISO8601(s, d2);
    equal(CM.Helpers.DateToISO8601(d2.v), CM.Helpers.DateToISO8601(d), "Date mismatch src "+s);
});